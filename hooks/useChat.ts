...
export function useChat() {
    ...
    // 채팅 히스토리를 메모리에 저장하지 않음 - 필요할 때마다 백엔드에서 조회
    const [chatList, setChatList] = useState<UIChatSession[]>([])
    ...
    // 현재 채팅방의 메시지만 메모리에 저장 (다른 채팅방과 분리)
    ...
    // 새 채팅 시작
    const handleNewChat = () => {
        console.log('[CHAT] 새 채팅 시작 - 상태만 초기화 (실제 생성은 메시지 전송 시)')
        
        // 상태만 초기화하고, 실제 채팅 생성은 메시지 전송 시에 처리
        setCurrentChatId(null)  // null로 설정하여 메시지 전송 시 새 채팅 생성되도록 함
        setCurrentMessages([])
        ...
        console.log('[CHAT] 새 채팅 준비 완료 - 메시지 전송 시 실제 생성됩니다')
    }
    ...
    const handleChatSubmit = async (message: string, image?: File) => {
        ...
        // 사용자 메시지는 백엔드에서 AI 응답과 함께 저장되므로 여기서는 저장하지 않음
        console.log('[CHAT] 사용자 메시지는 백엔드에서 저장됩니다')
        ...
        try {
            ...
            if (!raw || typeof raw !== "object") {
                ...
                if (parsedResponse.type === "recipe" && parsedResponse.recipes) {
                    ...
                    // 레시피는 백엔드에서 이미 저장되었으므로 여기서는 저장하지 않음 (이전 스키마)
                    console.log('[CHAT] 레시피는 백엔드에서 이미 저장되었습니다 (이전 스키마)')
                } else if (parsedResponse.type === "cart" && parsedResponse.ingredients) {
                    setCurrentView("cart");
                    setCurrentIngredients(parsedResponse.ingredients);
                    
                    // 카트 아이템은 백엔드에서 이미 저장되었으므로 여기서는 저장하지 않음 (이전 스키마)
                    console.log('[CHAT] 카트 아이템은 백엔드에서 이미 저장되었습니다 (이전 스키마)')
                } else {
                    setCurrentView("welcome");
                }
            ...
            // AI 메시지는 백엔드에서 이미 저장되었으므로 여기서는 저장하지 않음
            console.log('[CHAT] AI 메시지는 백엔드에서 이미 저장되었습니다')
    ...
    return {
        currentView,
        chatHistory: chatList, // chatList를 chatHistory로 반환 (기존 코드와 호환성 유지)
        currentChatId,
        currentMessages,
        isLoading,
        error,
