...
export class ChatService {
  ...
  async getChat(id: string): Promise<ChatRecord | undefined> {
    try {
      const response = await getJson<any>(`/api/chat/${id}`)
      // 백엔드 응답 구조에 맞게 수정
      ...
      return {
        ...
        title: data.chat?.title || "New Chat", // 채팅방 제목 추가
        ...
        recipes: [], // 백엔드에서 별도로 조회 필요
        cartItems: [] // 백엔드에서 별도로 조회 필요
      }
    } catch (error) {
